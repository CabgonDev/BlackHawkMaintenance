...
object GeneralitiesUpdateManager {

    private const val TAG = "GenUpdate"

    // SharedPrefs (mismo archivo que Frecuencias, keys distintas)
    private const val SP_FILE = "content_ota_prefs"
    private const val KEY_GEN_VERSION = "gen_version"
    private const val KEY_GEN_LAST_UPDATE_AT = "gen_last_update_at"

    // ✅ NUEVO: para soportar rollback y detectar cambios aunque versión sea igual
    private const val KEY_GEN_SHA = "gen_sha"
    private const val KEY_GEN_PATH = "gen_path"

    // Remote Config keys (mismo pipeline)
    ...
    suspend fun checkAndUpdate(
        ...
        if (!enabled) {
            push(Event.Skipped("Generalidades OTA deshabilitado por Remote Config."))
            return@withContext Result(updated = false, channelUsed = channel, events = events)
        }

        val spec = ContentOtaRepository.loadGeneralitiesSpec(channel)
        if (spec == null) {
            push(Event.Error("Generalidades", "No se pudo leer spec en Firestore (content_ota/channels)."))
            return@withContext Result(updated = false, channelUsed = channel, events = events)
        }

        val sp = sp(context)
        val localVer = sp.getLong(KEY_GEN_VERSION, 0L)
        val localSha = sp.getString(KEY_GEN_SHA, "")?.trim().orEmpty()
        val localPath = sp.getString(KEY_GEN_PATH, "")?.trim().orEmpty()

        val outFile = generalitiesFile(context)

        Log.d(
            TAG,
            "GEN: localVer=$localVer remoteVer=${spec.version} " +
                    "path=${spec.storagePath} bytes=${spec.bytes} sha=${shaShort(spec.sha256)} " +
                    "localSha=${shaShort(localSha)} localPath=${if (localPath.isBlank()) "—" else localPath} " +
                    "outFile=${fileInfo(outFile)}"
        )

        // ✅ Rollback-safe:
        // Solo es "UpToDate" si la versión es IGUAL (no <=).
        // Extra: si guardamos sha/path, también deben coincidir para saltar.
        ...
        if (sameVer && sameSha && samePath && fileLooksOk) {
            push(Event.UpToDate("Generalidades"))
            return@withContext Result(updated = false, channelUsed = channel, events = events)
        }

        push(Event.Downloading("Generalidades"))
        ...
        if (ok) {
            sp.edit()
                .putLong(KEY_GEN_VERSION, spec.version)
                .putString(KEY_GEN_SHA, spec.sha256 ?: "")
                .putString(KEY_GEN_PATH, spec.storagePath)
                .putLong(KEY_GEN_LAST_UPDATE_AT, System.currentTimeMillis())
                .apply()

            push(Event.Applied("Generalidades"))
            Log.d(TAG, "GEN: applied remoteVer=${spec.version} outFile=${fileInfo(outFile)}")
            Result(updated = true, channelUsed = channel, events = events)
        } else {
            push(Event.Error("Generalidades", "Descarga/verificación falló"))
            Log.w(TAG, "GEN: failed remoteVer=${spec.version} outFile=${fileInfo(outFile)}")
            Result(updated = false, channelUsed = channel, events = events)
        }
    }

    fun localVersion(context: Context): Long =
        sp(context).getLong(KEY_GEN_VERSION, 0L)

    fun generalitiesFile(context: Context): File {
        val dir = File(context.filesDir, "generalities").apply { mkdirs() }
        return File(dir, "generalities.json")
    }
